...
public class IMU implements Runnable {
	...
	public void run() {
		...
		while(true) {
			...
			MadgwickAHRSupdateIMU((float)(X_Gyro*DEGREES_TO_RAD), (float)(Y_Gyro*DEGREES_TO_RAD), (float)(Z_Gyro*DEGREES_TO_RAD), (float)X_Acel, (float)Y_Acel, (float)Z_Acel);
	...
	static final float sampleFreq =	100.0f;		// sample frequency in Hz
	static final double gyroMeasError = Math.PI * (5.0 / 180.0);
	static final double betaDef = gyroMeasError*Math.sqrt(3.0/4.0); //0.1f;		// 2 * proportional gain

	//---------------------------------------------------------------------------------------------------
	// Variable definitions

	volatile float beta = (float)betaDef;								// 2 * proportional gain (Kp)
	volatile float q0 = 1.0f, q1 = 0.0f, q2 = 0.0f, q3 = 0.0f;	// quaternion of sensor frame relative to auxiliary frame

	//http://www.chrobotics.com/library/understanding-quaternions	
	public double getXAngle() { //Roll
		return Math.atan2((2*(q0*q1 + q2*q3)),(q0*q0 - q1*q1 - q2*q2 + q3*q3))*RAD_TO_DEGREES;
	}
	
	public double getYAngle() { //Pitch
		return -1*Math.asin(2*(q1*q3 + q0*q2))*RAD_TO_DEGREES;
	}
	
	public double getZAngle() { //Yaw
		return Math.atan2((2*(q0*q3 + q1*q2)),(q0*q0 + q1*q1 - q2*q2 - q3*q3))*RAD_TO_DEGREES;
	}
